### Copiar projeto em C
### Criar condicional para verificar qual estado o CPF gerado pertence
#           
#    PARA VERIFICAR O ESTADO DE ONDE O CPF PERTENCE BASTA:
#    1) CPF: abc.def.ghi-jk verifica-se o numero na posição (i)
#    2) Exemplo: Nº:  003.939.708-41 = Código 8, corresponde ao Estado de São Paulo.
#    Código 0:  Rio Grande do Sul
#    Código 1:  Distrito Federal – Goiás – Mato Grosso – Mato Grosso do Sul – Tocantins
#    Código 2:  Pará – Amazonas – Acre – Amapá – Rondônia – Roraima
#    Código 3:  Ceará – Maranhão – Piauí
#    Código 4:  Pernambuco – Rio Grande do Norte – Paraíba – Alagoas
#    Código 5:  Bahia – Sergipe
#    Código 6:  Minas Gerais
#    Código 7:  Rio de Janeiro – Espírito Santo
#    Código 8:  São Paulo
#    Código 9: Paraná – Santa Catarina
#            
# TCP/IP Ilustreated, Volume 1: The Protocols (2nd edition) (addison-wesley professionel computing series)
# 
# Practical Packet Analysis, 3rd Edition: Using Wireshark to solve Real-World Network Problems
# Wireshark 101: Essential Skills for Network Analysis - Second Edition: Wireshark Solutions Series
# Lean Wireshark: Confidently navigate the Wireshark interface and solve real-world networking problems
#
#
#
#
#
# ____     ____    ____      ____                                          __                   
#/\  _`\  /\  _`\,/\  _`\   /\  _`\                                       /\ \__                
#\ \ \/\_\\ \ \L\ \ \ \L\_  \ \ \L\_\     __    ___      __   _ __    __  \ \ ,_\   ___   _ __  
# \ \ \/_/_\ \ ,__/\ \  _\/  \ \ \L_L   /'__`\/' _ `\  /'__`\/\`'__\/'__`\ \ \ \/  / __`\/\`'__\
#  \ \ \L\ \\ \ \/  \ \ \/    \ \ \/, \/\  __//\ \/\ \/\  __/\ \ \//\ \L\.\_\ \ \_/\ \L\ \ \ \/ 
#   \ \____/ \ \_\   \ \_\     \ \____/\ \____\ \_\ \_\ \____\\ \_\\ \__/.\_\\ \__\ \____/\ \_\ 
#    \/___/   \/_/    \/_/      \/___/  \/____/\/_/\/_/\/____/ \/_/ \/__/\/_/ \/__/\/___/  \/_/ 
#
#
#
#
#
#
#
#
#
#
#
#
#
#
#
#
#
#
#
#
#
#
#